...

{
	...
	function cargarTabla( opcion )
	{
		if( opcion == 1 )
		{
			$( "#cuerpo" ).load( "http://localhost:8081/tablet/buscarLinks" , { coincidencia : $( "#coincidencia" ).val() , type : 1 } , function(responseText, textStatus, XMLHttoRequest)
			{
		...
		{
			$( "#cuerpo" ).load( "http://localhost:8081/tablet/buscarLinks/index" , { valor : $( "#categoriaB option:selected" ).val() , type : 2 } , function(responseText, textStatus, XMLHttoRequest)
			{
				if(textStatus == "success")
				{
					editarDatos( 2 );
